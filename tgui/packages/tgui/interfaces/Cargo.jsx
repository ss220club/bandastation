          ...
          {cart.map((entry) => (
            <Table.Row key={entry.id} className="candystripe">
              ...
              <Table.Cell collapsing textAlign="right" inline ml="50px">
                ...
                {entry.dep_order > 0 && <b>[Отдел x {entry.dep_order}]</b>}
              </Table.Cell>
...
const CargoHelp = (props) => {
  return (
    <>
      <Section title="Заказы отделов">
        Каждый отдел на станции может заказать ящики с их собственных
        консолей. Эти заказы абсолютно БЕСПЛАТНЫ! Они не оплачиваются через
        бюджет карго, а имеют задержку на их консолях. Всё
        ...
        Вы даже получите полную стоимость ящика на счёт снабжения при
      ...
      <Section title="МУЛботы">
        МУЛботы - медленные, но лояльные роботы-доставки, которые могут доставить ящики
        с минимальным вмешательством техника. Это медлительный процесс и к ним могут
        вмешаться во время пути.
        <br />
        <b>Настроить МУЛбота довольно просто:</b>
        <br />
        <b>1.</b> Поставьте ящик, который вы хотите доставить рядом с МУЛботом.
        <br />
      ...
      <Section title="Система доставки по трубам">
        В дополнение к МУЛам и ручной доставке, вы можете использовать
        систему доставки по трубам. Поломки в трубах
        могут привести к потере посылки (такое происходит нечасто), потому, это не
        самый верный способ доставки. Вы также можете упаковать
        листок бумаги и отправить по трубам, если вы хотите (или кто-то на стойке)
        хотите просто отправить письмо.
        <br />
        <b>Использование системы доставки по трубам даже проще:</b>
        <br />
        <b>1.</b> Обмотайте свой предмет/ящик обёрточной бумагой.
        <br />
        <b>2.</b> Используйте маркировщик назначения для выбора места доставки.
        <br />
