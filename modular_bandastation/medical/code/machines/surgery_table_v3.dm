...
/obj/machinery/optable/v3/examine(mob/user)
	. = ..()
	. += "<hr>"

	. += span_info("The neural suppressors are switched [suppressing ? "on" : "off"].")

/obj/machinery/optable/v3/attack_hand(mob/user, act_intent, attackchain_flags)
...
/obj/machinery/optable/v3/process()
	if (!isnull(patient))
		recheck_patient(patient)
		if(patient?.loc != loc)
			stop_supress()
...
/obj/machinery/optable/v3/proc/stop_supress()
	suppressing = FALSE

/obj/machinery/optable/v3/Destroy()
	STOP_PROCESSING(SSobj, src)
	stop_supress()
	. = ..()

/obj/machinery/optable/v3/recheck_patient()
	. = .. ()
