...
/datum/reagent/medicine/omnidrite/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjustOrganLoss(ORGAN_SLOT_BRAIN, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_EYES, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_EARS, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_TONGUE, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_HEART, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_LIVER, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_LUNGS, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjustOrganLoss(ORGAN_SLOT_EYES, -0.05 * REM * seconds_per_tick * normalise_creation_purity())
