...
/datum/antagonist/bloodsucker/proc/break_masquerade(mob/admin)
	if(broke_masquerade)
		return
	owner.current.playsound_local(null, 'modular_bandastation/blood_suckers/sound/lunge_warn.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/bloodsucker/proc/return_vamp_examine(mob/living/viewer)
	...
	if(viewer.mind.has_antag_datum(/datum/antagonist/vassal) in vassals)
		...
		var/returnIcon = "[icon2html('modular_bandastation/blood_suckers/icons/vampiric.dmi', world, "bloodsucker")]"
		returnString += "\n"
