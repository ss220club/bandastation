...
/obj/item/gun/energy/laser/awaymission_aeg/proc/check_z()
	...
	if(onAwayMission())
		selfcharge = TRUE
		if(ismob(loc))
			to_chat(loc, span_notice("[capitalize(src.declent_ru(NOMINATIVE))] активируется, начиная аккумулировать энергию из материи сущего."))
	else
		selfcharge = FALSE
		cell.change(-STANDARD_BATTERY_CHARGE)
		update_appearance()
		if(ismob(loc))
			to_chat(loc, span_danger("[capitalize(src.declent_ru(NOMINATIVE))] деактивируется, так как он подавляется системами станции."))
