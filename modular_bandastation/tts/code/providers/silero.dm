...
/datum/tts_provider/silero/request(text, datum/tts_seed/silero/seed, datum/callback/postprocess_callback)
	if(throttle_check())
		return FALSE
	...
	SShttp.create_async_request(RUSTG_HTTP_METHOD_POST, CONFIG_GET(string/tts_api_url_silero), json_encode(req_body), list("content-type" = "application/json"), postprocess_callback)
