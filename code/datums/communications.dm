...
/datum/communciations_controller/proc/make_announcement(mob/living/user, is_silicon, input, syndicate, list/players)
	...
	if(is_silicon)
		...
		if(syndicate)
			priority_announce(html_decode(message_data["message"]), null, 'sound/announcer/announcement/announce_syndi.ogg', ANNOUNCEMENT_TYPE_SYNDICATE, has_important_message = TRUE, players = players, color_override = "red", tts_seed_override = user.get_tts_seed()) // Bandastation Addition: "tts_seed_override = user.get_tts_seed()"
		else
			priority_announce(html_decode(message_data["message"]), null, 'sound/announcer/announcement/announce.ogg', ANNOUNCEMENT_TYPE_CAPTAIN, has_important_message = TRUE, players = players, tts_seed_override = user.get_tts_seed()) // Bandastation Addition: "tts_seed_override = user.get_tts_seed()"
