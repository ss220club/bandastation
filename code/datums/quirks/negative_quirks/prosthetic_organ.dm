/datum/quirk/prosthetic_organ
	...
	desc = "В результате несчастного случая вы лишились одного из органов. Из-за этого у вас теперь есть запасной протез.!"
	icon = FA_ICON_LUNGS
	value = -3
...
/datum/quirk/prosthetic_organ/add_unique(client/client_source)
	...
	switch(organ_slot)
		...
		if(ORGAN_SLOT_STOMACH)
			...
			slot_string = "желудок"
	medical_record_text = "При физическом осмотре у пациента был обнаружен бюджетный протез [slot_string]. \
...
/datum/quirk/prosthetic_organ/post_add()
	to_chat(quirk_holder, span_boldannounce("Ваш [slot_string] была замен дешевым протезом. Он слаб и крайне нестабилен. \
