	...
	switch(blood_bled)
		...
		if(7 to 13)
			victim.visible_message(
				span_smalldanger("Капли крови вылетают из дыры в [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] [victim]!."),
				span_danger("Вы откашливаетесь, выплевывая немного крови из удара по вашей [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone]."),
				vision_distance = COMBAT_MESSAGE_RANGE,
			)
			victim.bleed(blood_bled, TRUE)
		if(14 to 19)
			victim.visible_message(
				span_smalldanger("Небольшая струя крови брызгает из дыры в [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] [victim]!!"),
				span_danger("Вы выплевываете струю крови из удара по вашей [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone]!"),
				vision_distance = COMBAT_MESSAGE_RANGE,
			)
		...
		if(20 to INFINITY)
			victim.visible_message(
				span_danger("Брызги крови струятся из раны в [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] [victim]!"),
				span_bolddanger("Вы задыхаетесь, выплевывая брызги крови из удара по вашей [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone]!"),
				vision_distance = COMBAT_MESSAGE_RANGE,
			)
			victim.bleed(blood_bled)
			new /obj/effect/temp_visual/dir_setting/bloodsplatter(victim.loc, victim.dir)
	...
	if(limb.can_bleed())
		if(victim.bodytemperature < (BODYTEMP_NORMAL - 10))
			...
			if(SPT_PROB(2.5, seconds_per_tick))
				to_chat(victim, span_notice("Вы чувствуете, как [LOWER_TEXT(name)] в вашей [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] становится твердым от холода!"))
...
/datum/wound/pierce/bleed/proc/suture(obj/item/stack/medical/suture/I, mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_WOUND_SCANNED))
		treatment_delay *= 0.5
		user.visible_message(span_notice("[user] начинает искусно зашивать [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] [victim] с помощью [I]..."), span_notice("Вы начинаете зашивать [user == victim ? "вашу" : "[victim]"] [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] с помощью [I], имея в виду информацию с голограммы..."))
	else
	...
	if(blood_flow > 0)
		...
		to_chat(user, span_green("Вы успешно закрываете дыру в [user == victim ? "вашей" : "[victim]'s"] [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone]."))
		return TRUE
...
/datum/wound/pierce/bleed/proc/tool_cauterize(obj/item/I, mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_WOUND_SCANNED))
		treatment_delay *= 0.5
		user.visible_message(span_danger("[user] начинает искусно прижигать [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] [victim] с помощью [I]..."), span_warning("Вы начинаете прижигать [user == victim ? "вашу" : "[victim]"] [limb.ru_plaintext_zone[NOMINATIVE] || limb.plaintext_zone] с помощью [I], учитывая информацию с голограммы..."))
	else
...
/datum/wound/pierce/bleed/severe
	...
	examine_desc = "проколотое насквозь, с кусочками ткани, закрывающими открытое отверстие"
	occur_text = "теряет сильный фонтан крови, обнажая проколотую рану"
...
/datum/wound/pierce/bleed/critical
	...
	examine_desc = "разорвано насквозь, едва удерживается на месте открытой костью"
