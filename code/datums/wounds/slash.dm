...
/datum/wound/slash/flesh/critical
	...
	simple_treat_text = "<b>Наложение повязки</b> на рану является важным и уменьшит потерю крови. После этого рану можно <b>зашить</b>, желательно, чтобы пациент отдыхал и/или держал свою рану. Следует немедленно обратиться за помощью - <b>Смерть</b> наступит, если лечение будет задержано, так как нехватка <b>кислорода</b> убивает пациента, поэтому <b>Питание, Железо и солевой раствор</b> всегда рекомендуется после лечения. Эта рана не заживет сама по себе."
	homemade_treat_text = "Простыни можно порвать, чтобы сделать <b>временный бинт</b>. <b>Мука, соль и соленая вода</b>, нанесенные сверху, помогут. Падение на землю и сжатие раны уменьшит кровотечение."
