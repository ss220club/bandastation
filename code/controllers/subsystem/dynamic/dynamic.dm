...
/datum/controller/subsystem/dynamic/proc/generate_advisory_level()
	...
	if(prob(PULSAR_REPORT_CHANCE))
		...
		advisory_string += "В вашем секторе установлена угроза уровня Пульсар. Большое и неизвестное электромагнитное поле прошло штормом через близлежащее оборудования наблюдения, вызвав значительную потерю данных. Часть данных была восстановлена, и она не выявляет достоверные угрозы для активов Нанотрейзен в секторе Спинвард; однако Департамент разведки рекомендует сохранять повышенную бдительность в отношении потенциальных угроз из-за отсутствия полных данных."
		return advisory_string
