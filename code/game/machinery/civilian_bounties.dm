...
/obj/item/bounty_cube/proc/set_up(datum/bounty/my_bounty, obj/item/card/id/holder_id)
	...
	COOLDOWN_START(src, next_nag_time, nag_cooldown)
	radio.talk_into(src,"Создан в [get_area(src)] пользователем [bounty_holder] ([bounty_holder_job]). Бонус за быструю доставку будет потерян через [time2text(next_nag_time - world.time,"mm:ss")].", RADIO_CHANNEL_SUPPLY)
