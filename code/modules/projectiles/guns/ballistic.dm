...
/obj/item/gun/ballistic/attackby(obj/item/A, mob/user, params)
	...
	if (isammocasing(A) || istype(A, /obj/item/ammo_box))
		...
		if (bolt_type == BOLT_TYPE_NO_BOLT || internal_magazine)
			...
			if (num_loaded)
				balloon_alert(user, "[cartridge_wording] заряжено: [num_loaded]")
				playsound(src, load_sound, load_sound_volume, load_sound_vary)
	...
	if(istype(A, /obj/item/suppressor))
		...
		if(!can_suppress)
			balloon_alert(user, "[S.declent_ru(NOMINATIVE)] не вставляется!")
			return
		if(!user.is_holding(src))
			balloon_alert(user, "нужно держать в руке!")
			return
		if(suppressed)
			balloon_alert(user, "уже имеется глушитель!")
			return
		if(user.transferItemToLoc(A, src))
			balloon_alert(user, "вставка [S.declent_ru(GENITIVE)]")
...
/obj/item/gun/ballistic/attack_self(mob/living/user)
	...
	if(bolt_type == BOLT_TYPE_NO_BOLT)
		...
		if (num_unloaded)
			balloon_alert(user, "[cartridge_wording] разряжено: [num_unloaded]")
