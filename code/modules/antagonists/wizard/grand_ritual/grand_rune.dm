...
/obj/effect/grand_rune
	...
	var/static/list/possible_magic_words = list(
		...
		list("Свифткаст! Хастега! Abjurer's Ward 2! Продление 4! Продвинутое усиление Тенсера! Защита от добра! Усиление эффекта 3! Аркановое ус...",
			"...иление 4! Превращение паразитов 10! Защита от зла 2! Щит мага! Посредственное усиление Венериуса 2! Расширить силу! Изгнать го...",
			"...лод 2! Защита от нейтралов! Сурекастага! Освежить! Освежить 2! Шарпкаст 10! Эфирная манипуляция! Поглощение Лей-линии! Вызвать Великий Ритуал!!"),
		list("Тен!", "Чи!", "Джи!"),
...
/obj/effect/grand_rune/proc/invoke_rune(mob/living/user)
	...
	if(!do_after(user, invoke_time, src))
		...
		user.balloon_alert(user, "преврано!")
		is_in_use = FALSE
		return
...
/obj/effect/grand_rune/proc/summon_round_event(mob/living/user)
	...
	if (!length(possible_events))
		visible_message(span_notice("[src] издает печальный свистящий звук."))
		return
