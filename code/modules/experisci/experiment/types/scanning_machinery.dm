...
/datum/experiment/scanning/points/machinery_pinpoint_scan/check_progress()
	. = ..()
	.[1] = EXPERIMENT_PROG_INT("Отсканируйте образцы следующих машин, улучшенных с \a [initial(required_stock_part.name)], чтобы набрать достаточное количество очков для завершения эксперимента.", points, required_points)[1]
