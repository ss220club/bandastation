...
/datum/round_event/radiation_leak/announce(fake)
	...
	priority_announce("Утечка радиации обнаружена в [location_descriptor || "an unknown area"]. \
		...
		указывают что машинерия в этой локации является причиной проблемы - отремонтируйте её как можно быстрее чтобы остановить утечку", "Инженерный отдел [command_name()]")
