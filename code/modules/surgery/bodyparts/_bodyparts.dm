	...
	for(var/datum/wound/wound as anything in wounds)
		switch(wound.severity)
			...
			if(WOUND_SEVERITY_SEVERE)
				check_list += "\t [span_boldwarning("Ваша [name] страдает от [wound.a_or_from] [LOWER_TEXT(wound.name)]!!")]"
			if(WOUND_SEVERITY_CRITICAL)
				check_list += "\t [span_boldwarning("Ваша [name] страдает от [wound.a_or_from] [LOWER_TEXT(wound.name)]!!!")]"

	for(var/obj/item/embedded_thing in embedded_objects)
		var/stuck_word = embedded_thing.is_embed_harmless() ? "проникает" : "впился"
		check_list += "\t <a href='?src=[REF(examiner)];embedded_object=[REF(embedded_thing)];embedded_limb=[REF(src)]' class='warning'>[embedded_thing] [stuck_word] в вашу [name]!</a>"
