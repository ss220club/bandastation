...
/mob/living/basic/parrot/poly/get_static_list_of_phrases() // there's only one poly, so there should only be one ongoing list of phrases. i guess
	...
	phrases_to_return += list(
		...
		":e Подключите соляры, ленивые бездельники!",
		"Поли хочет крекер!",
	)
	switch(determine_special_poly())
		...
		if(POLY_LONGEST_SURVIVAL)
			phrases_to_return += pick("...[longest_survival].", "Вещи, которые я видел...!", "Я прожил множество жизней!", "Кто вы передо мной?")
		if(POLY_BEATING_DEATHSTREAK)
			phrases_to_return += pick("Чего вы ждёте?!", "Насилие порождает насилие!", "Кровь! Кровь!!!", "Убейте меня, если хотите!")
...
/mob/living/basic/parrot/poly/update_desc()
	...
	switch(determine_special_poly())
		...
		if(POLY_BEATING_DEATHSTREAK)
			desc += " Визги [-rounds_survived] мёртвых попугаев раздаётся звоном в ваших ушах..."
		if(POLY_CONSECUTIVE_ROUND)
