...
/datum/mod_link/proc/get_other()
	...
	if(!link_call)
		return
	return link_call._caller == src ? link_call.receiver : link_call._caller
...
/datum/mod_link_call
	...
	var/datum/mod_link/_caller
	/// The MODlink that is being called.
	var/datum/mod_link/receiver

/datum/mod_link_call/New(datum/mod_link/_caller, datum/mod_link/receiver)
	_caller.link_call = src
	receiver.link_call = src
	src._caller = _caller
	src.receiver = receiver
	var/mob/living/_caller_mob = _caller.get_user_callback.Invoke()
	ADD_TRAIT(_caller_mob, TRAIT_IN_CALL, REF(src))
...
/datum/mod_link_call/Destroy()
	var/mob/living/_caller_mob = _caller.get_user_callback.Invoke()
	if(!QDELETED(_caller_mob))
		REMOVE_TRAIT(_caller_mob, TRAIT_IN_CALL, REF(src))
	var/mob/living/receiver_mob = receiver.get_user_callback.Invoke()
	if(!QDELETED(receiver_mob))
		REMOVE_TRAIT(receiver_mob, TRAIT_IN_CALL, REF(src))
	STOP_PROCESSING(SSprocessing, src)
	clear_visuals()
	_caller.link_call = null
...
/datum/mod_link_call/proc/can_continue_call()
	return _caller.frequency == receiver.frequency && _caller.can_call_callback.Invoke() && receiver.can_call_callback.Invoke()

/datum/mod_link_call/proc/make_visuals()
	var/_caller_visual = _caller.make_visual_callback.Invoke()
	var/receiver_visual = receiver.make_visual_callback.Invoke()
	_caller.get_visual_callback.Invoke(receiver_visual)
	receiver.get_visual_callback.Invoke(_caller_visual)

/datum/mod_link_call/proc/clear_visuals()
	_caller.delete_visual_callback.Invoke()
	receiver.delete_visual_callback.Invoke()

/proc/call_link(mob/user, datum/mod_link/calling_link)
	if(!calling_link.frequency)
		return
	var/list/_callers = list()
	for(var/id in GLOB.mod_link_ids)
		...
		if(!link.can_call_callback.Invoke())
			continue
		_callers["[link.holder] ([id])"] = id
	if(!length(_callers))
		calling_link.holder.balloon_alert(user, "no targets on freq [calling_link.frequency]!")
		return
	var/chosen_link = tgui_input_list(user, "Choose ID to call from [calling_link.frequency] frequency", "MODlink", _callers)
	if(!chosen_link)
		return
	calling_link.call_link(GLOB.mod_link_ids[_callers[chosen_link]], user)
...
/atom/movable/screen/alert/modlink_call/Click(location, control, params)
	...
	if(usr != owner)
		return
	var/datum/mod_link/_caller = _caller_ref.resolve()
	var/datum/mod_link/receiver = receiver_ref.resolve()
	if(!_caller || !receiver)
		return
	if(_caller.link_call || receiver.link_call)
		return
	var/list/modifiers = params2list(params)
	if(LAZYACCESS(modifiers, RIGHT_CLICK))
		end_message = "call denied!"
		owner.clear_alert("[REF(_caller)]_modlink")
		return
	end_message = "call accepted"
	new /datum/mod_link_call(_caller, receiver)
	owner.clear_alert("[REF(_caller)]_modlink")
...
/atom/movable/screen/alert/modlink_call/Destroy()
	...
	if(!user || !_caller)
		return ..()
	_caller.holder.balloon_alert(user, end_message)
	return ..()
